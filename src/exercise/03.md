...
1. What does it mean to lift the state? It means sharing state across sibling
   components by lifting the state up to their closest common ancestor (parent
   component)
2. This is as opposed to colocting state, which is where you have lifted state
   and a component does not need access to that state, leaving more to be
   maintained and making React re-render unnecessarily. Colocating state is
   removing it from an ancestor and using state only in the components that need
   it
3. What is prop drilling (also called 'threading')? The process you have to go
   through to get data to parts of the React component tree.
4. What are the pros & cons of prop drilling?
5. What is context? Inserting some state into a section of our react tree, then
   extracting that state anywhere within that react tree without having to
   explicity pass it everywhere. It is used as a solution to prop drilling
6. What are the steps of using context? One: create context, two:
   useContext(pass in the context name as an argument), three: pass it as a
   provider in the react tree so all components wrapped in it have access to
   that state
